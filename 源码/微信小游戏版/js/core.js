 ...
 * 源码指纹: 10a0d6f00fcb   （只跟 neon_maze_fragment.html 的内容走）
 */
...
// 展示偏好独立存储：不改变身份、进度和旧版静音存档协议。
const PRESENTATION_KEY = 'doudou.presentation.v1';
const PRESENTATION_DEFAULTS = {colorAssist:false,reduceMotion:'system',screenShake:true,effectVolume:1,alertVolume:1};
function normalizePresentation(raw){
  const clean = {...PRESENTATION_DEFAULTS};
  if (!saveObject(raw)) return clean;
  for (const key of ['colorAssist','screenShake']) if (typeof raw[key] === 'boolean') clean[key]=raw[key];
  if (raw.reduceMotion === 'reduce') clean.reduceMotion='reduce';
  for (const key of ['effectVolume','alertVolume']){
    if (typeof raw[key] === 'number' && Number.isFinite(raw[key])) clean[key]=Math.max(0,Math.min(1,raw[key]));
  }
  return clean;
}
function readPresentation(fallback=PRESENTATION_DEFAULTS){
  const raw=readStoredJSON(PRESENTATION_KEY);
  return saveObject(raw) ? normalizePresentation(raw) : {...fallback};
}
let presentation = readPresentation();
let presentationSaved = true;
function setPresentation(key,value){
  if (!Object.prototype.hasOwnProperty.call(PRESENTATION_DEFAULTS,key)) return;
  presentation=normalizePresentation({...readPresentation(presentation),[key]:value});
  try {localStorage.setItem(PRESENTATION_KEY,JSON.stringify(presentation));presentationSaved=true;}
  catch(e){presentationSaved=false;}
  applyPresentation();
}
...
const Audio2 = (()=>{
  ...
  let audioChannel = 'effectVolume';
  let muted = localSave.settings.muted;
  /* `muted` 是玩家自己的选择，会存下来；`silenced` 是外部临时把声音摁住
     （目前只有 CrazyGames 版本播广告时用），绝不落盘 —— 广告放完必须原样
     回到玩家的选择，而不是把人家的音量设置改掉。 */
  let silenced = false;
  /* 吃豆声固定在开局这一对（A4/C5），高低交替出「哒-嘀」的节奏。

     原来它是五档音阶，随本关吃掉的比例一路升上去（最高到 G5/C6），本意是
     让耳朵也能当进度条用。实际效果是：越到收尾音越高、越短、越密，业主
     反馈「很刺耳」—— 而这游戏的听众是小孩，一局最后那几十颗豆正是他们
     最专注的时候，不该在那时候让声音变得难受。
     进度反馈交给画面（剩余豆数提示、最后几颗的强调）就够了。 */
  const PELLET_TONE = [440, 523];
  function ctx(){
    if (!actx){
  ...
  function tone(freq, t0, dur, {type='square', gain=0.16, slideTo=null, slideDur=null}={}){
    const ac = ctx(); if (!ac || muted || silenced) return;
    gain *= presentation[audioChannel];
    if (!(gain > 0) || !Number.isFinite(freq) || freq <= 0) return;
  ...
  const sounds = {
    ...
    /** 临时静音，不写存储、不影响静音按钮的显示。 */
    setSilenced(v){ silenced = !!v; },
     ...
     */
    pellet(){
      /* 音高、时长、音量三者都不再随进度变。原来那三条渐变是配套的
         （升调的同时缩短、压低音量来抵消高频的刺耳），去掉升调之后
         另外两条也就没有存在的理由了。 */
      const t = now();
      pelletToggle = 1 - pelletToggle;
      tone(PELLET_TONE[pelletToggle], t, 0.058, {type:'triangle', gain:0.115});
    },
    ...
    /** x10 / x20 / 之后每 50：提醒无上限，音阶和音符数始终只用三档。 */
    comboMilestone(m){
      const t=now();
  ...
  for (const name of ['power','eatGhost','sweep','fruit','comboMilestone','phaseTick','phaseEnd','death','levelUp','gameOver','victory','newBest']){
    const play=sounds[name];
    sounds[name]=function(...args){
      const previous=audioChannel;audioChannel='alertVolume';
      try {return play.apply(this,args);} finally {audioChannel=previous;}
    };
  }
  return sounds;
})();
...
function checkPortal(ent){
  ...
  for (const p of pair){
    ...
    if (ent===player){
      Audio2.warp();
      emitFeedback('warp',cx,cy);emitFeedback('warp',ent.x,ent.y);
 ...
 *
 * 三条规矩：
 *   1 10 / 20 / 50 / 100 / 150……持续提醒；50 起音效强度封顶，不无限加码
 ...
 *     开新一局（fullReset）时清零。
 */
const COMBO_MILESTONE_STEP = 50;

function checkComboMilestone(){
  // 正常逐个续连击；即使恢复到跨越多档的值，也只报最高档，不排队轰炸。
  const m = combo >= COMBO_MILESTONE_STEP
    ? Math.floor(combo / COMBO_MILESTONE_STEP) * COMBO_MILESTONE_STEP
    : combo >= 20 ? 20 : combo >= 10 ? 10 : 0;
  if (!Number.isFinite(m) || m <= comboMilestoneHit) return;
  comboMilestoneHit = m;
...
function drawDeathFlash(){
  ...
  ctx.globalAlpha = prefersReducedMotion() ? 0.25 : 0.55 * k;
  ctx.strokeStyle = cssVar('--danger');
  ctx.shadowColor = cssVar('--danger');
  ctx.shadowBlur = prefersReducedMotion() ? 0 : 34;
  ctx.lineWidth = 10;
...
const prefersReducedMotion = () => presentation.reduceMotion === 'reduce' || !!systemMotionQuery?.matches;
...
// 新增纯视觉效果使用独立序列，不多消耗敌人决策所用的 Math.random。
...
function visualRandom(){
  ...
  return (visualRandomState >>> 0) / 4294967296;
}

// 轻量事件火花复用主画布；独立视觉时钟，不改变模拟时间/碰撞/角色坐标。
const FEEDBACK_LIMIT = 48;
const FEEDBACK_STYLES = {
  counter:{count:10,color:'#fff0a0',shake:1.5},warp:{count:6,color:'#7aeaff',shake:.6},
  phase:{count:8,color:'#d5a5ff',shake:.8},power:{count:8,color:'#ffe078',shake:.8},
  level:{count:10,color:'#ffe078',shake:1},
};
let feedbackParticles = [], shakeUntil = 0, shakeStrength = 0, lastShakeTransform = '';
function setShakeTransform(value){
  if (!CAN_OWN_CANVAS || value === lastShakeTransform) return;
  canvas.style.transform=value;lastShakeTransform=value;
}
function clearFeedback(){feedbackParticles=[];shakeUntil=0;shakeStrength=0;setShakeTransform('');}
function emitFeedback(kind,x,y){
  const style=FEEDBACK_STYLES[kind];
  if (!style || prefersReducedMotion() || !Number.isFinite(x) || !Number.isFinite(y)) return;
  const born=performance.now();
  for (let i=0;i<style.count && feedbackParticles.length<FEEDBACK_LIMIT;i++){
    const angle=visualRandom()*Math.PI*2,speed=14+visualRandom()*22;
    feedbackParticles.push({x:x*TILE+TILE/2,y:y*TILE+TILE/2,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,born,color:style.color});
  }
  if (presentation.screenShake){shakeUntil=born+140;shakeStrength=Math.max(shakeStrength,style.shake);}
}
function drawFeedback(){
  if (!feedbackParticles.length && !shakeUntil) return;
  const time=performance.now();
  if (prefersReducedMotion()){if(feedbackParticles.length || shakeUntil)clearFeedback();return;}
  let kept=0;
  ctx.save();ctx.shadowBlur=0;
  for (const p of feedbackParticles){
    const age=(time-p.born)/1000;if(age>=.28)continue;
    feedbackParticles[kept++]=p;
    ctx.globalAlpha=.65*(1-age/.28);ctx.fillStyle=p.color;
    ctx.fillRect(p.x+p.vx*age-1,p.y+p.vy*age-1,2,2);
  }
  feedbackParticles.length=kept;ctx.restore();
  if (presentation.screenShake && time<shakeUntil){
    const k=(shakeUntil-time)/140,amount=shakeStrength*k;
    setShakeTransform('translate('+ (Math.sin(time*.08)*amount).toFixed(2)+'px,'+(Math.cos(time*.11)*amount*.6).toFixed(2)+'px)');
  } else {shakeUntil=0;shakeStrength=0;setShakeTransform('');}
}

function startComboFx(milestone=50){
  if (prefersReducedMotion()) return;
  if (fxRunning) return;
  fxCanvas.classList.add('on');
  const { w, h } = fxResize();
  ...
  const n = 26;
  // 已有 10/20/50 档保留原随机调用；新增档位不扰动现有敌人随机序列。
  const random = milestone > 50 ? visualRandom : Math.random;
  for (let i=0;i<n;i++){
    fxParticles.push({
      x: w * (0.08 + random()*0.84),
      y: -4 - random()*10,
      vx: (random()-0.5)*36,
      vy: 30 + random()*50,
      life: 1.1,
      color: FX_COLORS[Math.floor(random()*FX_COLORS.length)],
      size: 1.8 + random()*1.8,
    });
  }
  fxLoopStart(w, h);
 ...
 */
/* 渠道后缀由各渠道的构建脚本注入 window.NEON_CHANNEL。自己的域名不注入，
   写出来的值和以前一模一样。不打标记就永远分不清哪个渠道真的有人在玩 ——
   榜是三个渠道共用的。数据库对这一列的约束是 1..32 字符，故截断兜底。 */
const CLIENT_VERSION = ('web-2026.09.04' + (
  typeof window.NEON_CHANNEL === 'string' && /^[a-z0-9-]{1,12}$/.test(window.NEON_CHANNEL)
    ? '-' + window.NEON_CHANNEL : '')).slice(0, 32);
const Analytics = {
  track(event, params={}){
    try {
...
function updateFruit(dt){
  ...
  if (fdx*fdx + fdy*fdy < 0.36){   // 0.6 格，平方比较不开根号
    ...
    Audio2.fruit();
    emitFeedback('phase',player.x,player.y);
  }
}
...
function currentScreen(){
  if (prefsOverlay && !prefsOverlay.classList.contains('hidden')) return 'preferences';
...
window.addEventListener('keydown', (e)=>{
  ...
  if (e.key==='Escape'){ closeHelp(); closeAbout(); closeOwl(); closePreferences(); return; }
...
if (prefsOverlay) prefsOverlay.classList.add('hidden'); // 原生垫片没有 HTML 初始 class
...
function docPanelOpen(){
  return !helpOverlay.classList.contains('hidden')
      ...
      || !!(prefsOverlay && !prefsOverlay.classList.contains('hidden'));
}

function syncPreferenceControls(){
  const set=(id,prop,value)=>{const el=document.getElementById(id);if(el)el[prop]=value;};
  set('prefColor','checked',presentation.colorAssist);
  set('prefMotion','value',presentation.reduceMotion);
  set('prefShake','checked',presentation.screenShake);
  for (const [id,key] of [['prefEffects','effectVolume'],['prefAlerts','alertVolume']]){
    const value=Math.round(presentation[key]*100);
    set(id,'value',String(value));set(id+'Value','textContent',value+'%');
  }
  set('prefsStatus','textContent',presentationSaved ? '设置只保存在当前浏览器。' : '浏览器未允许保存；设置仅本次生效。');
}
function applyPresentation(){
  const reduce=prefersReducedMotion();
  document.body.classList.toggle('reduce-motion',reduce);
  document.body.classList.toggle('color-assist',presentation.colorAssist);
  if (reduce){stopFireworks();clearFeedback();}
  else if (!presentation.screenShake){shakeUntil=0;shakeStrength=0;setShakeTransform('');}
  staticFrameDirty=true;
  syncPreferenceControls();
  document.querySelectorAll('[data-enemy]').forEach(el=>el.setAttribute('data-mark',enemyMark(el.dataset.enemy)));
}
function openPreferences(){
  if (!prefsOverlay || !prefsOverlay.classList.contains('hidden')) return;
  closeHelp();closeAbout();closeOwl();
  prefsOpener=document.activeElement;
  if (gameState==='playing'){
    resetJoystick();gameState='paused';document.getElementById('pauseOverlay').classList.remove('hidden');
  }
  presentation=readPresentation(presentation);applyPresentation();
  prefsOverlay.classList.remove('hidden');
  if (typeof prefsOverlay.focus==='function') prefsOverlay.focus({preventScroll:true});
}
function closePreferences(){
  if (!prefsOverlay || prefsOverlay.classList.contains('hidden')) return;
  prefsOverlay.classList.add('hidden');
  if (prefsOpener?.isConnected !== false && typeof prefsOpener?.focus==='function') prefsOpener.focus({preventScroll:true});
  prefsOpener=null; // 不自动继续，玩家自己恢复
}
['startPrefsBtn','pausePrefsBtn'].forEach(id=>document.getElementById(id)?.addEventListener('click',openPreferences));
document.getElementById('prefsCloseBtn')?.addEventListener('click',closePreferences);
for (const [id,key] of [['prefColor','colorAssist'],['prefShake','screenShake']]){
  const el=document.getElementById(id);el?.addEventListener('change',()=>setPresentation(key,el.checked));
}
const motionSelect=document.getElementById('prefMotion');
motionSelect?.addEventListener('change',()=>setPresentation('reduceMotion',motionSelect.value));
for (const [id,key] of [['prefEffects','effectVolume'],['prefAlerts','alertVolume']]){
  const el=document.getElementById(id);
  const value=()=>Math.max(0,Math.min(1,Number(el.value)/100));
  el?.addEventListener('input',()=>{presentation[key]=value();document.getElementById(id+'Value').textContent=Math.round(value()*100)+'%';});
  el?.addEventListener('change',()=>setPresentation(key,value()));
}
if (prefsOverlay) prefsOverlay.addEventListener('keydown',e=>{
  if(e.key!=='Tab')return;
  const first=document.getElementById('prefColor'),last=document.getElementById('prefsCloseBtn'),a=document.activeElement;
  if(e.shiftKey && (a===first || a===prefsOverlay)){e.preventDefault();last.focus();}
  else if(!e.shiftKey && (a===last || a===prefsOverlay)){e.preventDefault();first.focus();}
});
if(systemMotionQuery){
  if(typeof systemMotionQuery.addEventListener==='function')systemMotionQuery.addEventListener('change',applyPresentation);
  else if(typeof systemMotionQuery.addListener==='function')systemMotionQuery.addListener(applyPresentation);
}
window.addEventListener('storage',e=>{
  if(e.key!==PRESENTATION_KEY && e.key!==null)return;
  presentation=readPresentation();applyPresentation();
});
const ENEMY_MARKS={chaser:'A',ambush:'B',shy:'C',patrol:'D'};
function enemyMark(id){return ENEMY_MARKS[id] || '?';}
function drawContrastMark(mark,x,y){
  ctx.save();ctx.shadowBlur=0;ctx.globalAlpha=1;
  ctx.fillStyle='#080519';ctx.fillRect(x-5.5,y-6,11,12);
  ctx.strokeStyle='#fff';ctx.lineWidth=.8;ctx.strokeRect(x-5.5,y-6,11,12);
  ctx.fillStyle='#fff';ctx.font='bold 9px system-ui';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(mark,x,y+.5);
  ctx.restore();
}
...
function update(dt){
  ...
  if (pelletsLeft<=0){
    ...
    Audio2.levelUp();
    emitFeedback('level',player.x,player.y);
  }
}
...
function drawMaze(){
  ...
  if (pelletsLeft <= LAST_PELLET_HINT){
    ...
    const beat = reduce ? 1 : (Math.sin(elapsed * (5 + urgency*3)) + 1) / 2;   // 0..1
    glow = PELLET_GLOW + beat * (PELLET_GLOW_SWING * urgency + PELLET_GLOW_BASE_SWING);
  }
  const amber = cssVar('--amber');
  const pulse = POWER_R + (reduce ? 0 : Math.sin(elapsed*6) * POWER_R_SWING);
  let anyPower = false;
  ...
  if (anyPower){
    ...
    ctx.globalAlpha = reduce ? .88 : .68 + .2*Math.sin(elapsed*6);
    ctx.lineWidth = 1.2;
    ctx.stroke();
    ctx.globalAlpha = 1;
  }
  ...
  for (const ch of ['1','2']){
    for (const p of portalTiles[ch]){
      ...
      const col = ch==='1' ? cssVar('--cyan') : cssVar('--pink');
        ...
        const portalAngle = cooling || reduce ? 0 : -elapsed*2.4;
        ...
        if (presentation.colorAssist){ctx.rotate(-portalAngle);drawContrastMark(ch,0,0);}
        ctx.restore();
    }
  }
  ctx.shadowBlur=0;
...
function drawFruit(){
  if (!fruit.active) return;
  const cxp = fruit.x*TILE+TILE/2, cyp = fruit.y*TILE+TILE/2 + (prefersReducedMotion() ? 0 : Math.sin(elapsed*8)*2);
  ctx.save();
  ctx.translate(cxp,cyp);
  ctx.fillStyle = cssVar('--cyan');
...
function drawPlayer(){
  ...
  const wind = (momentumMult() - 1) / (MOMENTUM_MAX - 1); // 0..1
  if (wind > 0.15 && player.trail && player.trail.length && !reduce){
    ctx.save();
  ...
  if (player.chompT !== undefined && player.chompT < 1){
    ...
    joy = reduce ? 0 : Math.sin(player.chompT * Math.PI);
  }
  /* 无敌和穿墙都靠"闪"来表示，但原来两处都是**硬切**：穿墙 5Hz、无敌 7Hz，
     ...
     难受得多。 */
  const pulse = (hz, lo) => reduce ? 1 : lo + (1 - lo) * (0.5 + 0.5 * Math.sin(elapsed * hz * 2 * Math.PI));
  ...
  const stride = moving && !reduce ? Math.sin(gaitPhase) : 0;
  ...
  const turning = !reduce && player.visualTurnT < 1 ? Math.sin(player.visualTurnT*Math.PI) : 0;
  ctx.save();
  ctx.translate(cxp-player.dir.y*sway,
                cyp+player.dir.x*sway*.35+hop-joy*.45);
  ctx.rotate(reduce ? 0 : player.visualLean+player.visualTurnSign*turning*.06);
...
function drawGhost(g){
  ...
  if (g.state==='eaten'){
    ...
    const ending = frightTimer < 1.8 && (reduce || Math.sin(elapsed * GHOST_WARNING_HZ * Math.PI * 2) > 0);
  ...
  if (!eyesOnly && drawCharacterSprite(g.id,ENEMY_SPRITE_SIZE,spriteMode)){
    ...
    if (presentation.colorAssist) drawContrastMark(enemyMark(g.id),10,11);
    ctx.restore();
    return;
  }
  if (!eyesOnly){
  ...
  if (eyesOnly){
    ...
    ctx.save();ctx.rotate(reduce ? 0 : elapsed*2.4);
  ...
  if (presentation.colorAssist && !eyesOnly) drawContrastMark(enemyMark(g.id),10,11);
  ctx.restore();
}
...
function stepFrame(dt){
  if (gameState!=='playing' && (feedbackParticles.length || shakeUntil)){
    clearFeedback();staticFrameDirty=true;
  }
...
window.NeonGame = {
  ...
  resume: () => { if (gameState === 'paused') document.getElementById('resumeBtn').click(); },
  /* 平台适配层专用：播广告时临时闭嘴，放完恢复。不碰玩家的静音设置。 */
  silence: v => Audio2.setSilenced(v),
  start: () => { if (gameState === 'ready') document.getElementById('startBtn').click(); },
  challenge: () => {
