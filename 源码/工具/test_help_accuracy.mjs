...
const enChecks = [
  ...
  ['固定奖励与旧成绩', 'fixed bonuses and existing scores stay unchanged'],
  ['豆子', `<dt>Dot</dt><dd><b>${plainNum(pelletPoints)}</b> × Combo, then rounded`],
  ['能量星', `<dt>Power Star</dt><dd><b>${plainNum(powerPoints)}</b> × Combo, then rounded`],
  ['相位晶石', `<dt>Phase Crystal</dt><dd><b>${plainNum(fruitPoints)}</b> × Combo, then rounded`],
  ['敌人悬赏', `<dt>Enemy</dt><dd><b>${plainNum(bountyPoints/1000)}k → ${plainNum(bountyPoints*2/1000)}k → ${plainNum(bountyPoints*3/1000)}k`],
  ['整关无伤', `<dt>Perfect level</dt><dd><b>${plainNum(bonus('PERFECT_LEVEL')*MULT)}</b> × level`],
  ['全灭对手', `<dt>Enemy sweep</dt><dd><b>${plainNum(bonus('GHOST_SWEEP')/1000)}k</b>`],
  ...
  ['六关能量时长', `<dt>Power Star duration</dt><dd><b>${fright}s on all six levels</b>`],
  ['续星刷新', `another Power Star resets the timer to ${fright}s`],
  ['六关敌人速度', `<dt>Enemy speed</dt><dd><b>${ghostBaseSpeed} tiles/s</b> base speed on all six levels`],
  ['冲刺倍率', `<b>${dash}×</b>`],
  ['玩家加速', `you move ${Math.round((pSpeed-1)*100)}% faster`],
  ['敌人减速', `enemies ${Math.round((1-gSpeed)*100)}% slower`],
  ['传送门冷却', `<dt>Portal</dt><dd><b>${portalCd}s</b> cooldown`],
  ['统一连击', 'Dots, Power Stars, countered enemies and crystals all extend the <b>same combo</b>, adding one level'],
  ['独立敌人悬赏', 'enemies also extend the same combo, but their bounties are not multiplied by Combo'],
];
