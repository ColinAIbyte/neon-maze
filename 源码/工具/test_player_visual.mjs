...
if (!src.includes("assets/neon-stalkers-contrast-v4.webp") || !/const\s+sw=aw\/2,sh=ah\/2/.test(sprite))
  fail.push('四只高对比异形大眼追猎怪的 2×2 图集没有接入');
if (!/ambush:\{w:1,h:1\}/.test(src) || /ambush:\{w:1\.22/.test(src))
  fail.push('拦拦仍被横向放大，30px 图框也会重新挤进墙体');
if (!/wx\.createImage\(\)/.test(src) || !/images\/neon-stalkers-contrast-v4\.webp/.test(src))
