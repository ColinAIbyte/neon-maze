 ...
 * 源码指纹: 991eaaf293e7   （只跟 pacman_fragment.html 的内容走）
 */
...
/* 尾焰永远拖在身后，是四方向最醒目的不对称标记。 */
function drawDoudouTail(facing){
  const f = DOUDOU_FACING[facing] || DOUDOU_FACING.down;
  const bx=-f.x*8.1, by=-f.y*7.1+1.2;
  const sx=-f.y, sy=f.x;
  ctx.save();
  ctx.globalAlpha*=.9;
  ctx.fillStyle='#86ffe2';ctx.shadowColor=cssVar('--cyan');ctx.shadowBlur=5;
  ctx.beginPath();
...
function drawDoudouDiamond(x,y,size){
  ctx.save();ctx.translate(x,y);ctx.rotate(Math.PI/4);
  ctx.fillStyle='#e9ffff';ctx.shadowColor=cssVar('--cyan');ctx.shadowBlur=5;
...
function drawDirectionalDoudouFace(facing, joy){
  ctx.save();ctx.shadowBlur=0;
  if (facing==='up'){
    ctx.fillStyle='#157c86';
    fillDoudouOval(0,-.4,5.6,6.6);
    ctx.strokeStyle='#9fffea';ctx.lineWidth=.85;ctx.lineCap='round';
    ctx.beginPath();ctx.arc(0,-.3,4.1,.15*Math.PI,.85*Math.PI);ctx.stroke();
    drawDoudouDiamond(0,2.2,2.6);
  } else if (facing==='left' || facing==='right'){
    const side=facing==='right'?1:-1;
    /* 向前突出的脸和单眼让左右转向一眼可读；另一只眼被身体挡住。 */
    ctx.fillStyle='#45d9c7';
    ...
    ctx.stroke();
    drawDoudouDiamond(-side*1.4,4.9,2.15);
  } else {
...
function drawPlayer(){
  ...
  ctx.fill();
  ctx.shadowBlur=0;ctx.strokeStyle='rgba(207,255,247,.72)';ctx.lineWidth=.72;
  ctx.stroke();
  ...
  drawDirectionalDoudouFace(facing,joy);
